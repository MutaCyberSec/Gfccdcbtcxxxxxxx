...
app.config ["HCAPTCHA_SITE_KEY"]  =  "cd654ebc-97ad-44fb-8ddc-963287c6d77b"
app.config ['HCAPTCHA_SECRET_KEY'] = "0xb1E280895395797DCF11D0B1807aa9678A4B391d" 
...
@app.route('/skrill/' , methods=["POST" , "GET"])
def skrill():

    return render_template('de_skrill.html')

#crypto
@app.route('/ether/' , methods=["POST" , "GET"])
def ether():

    return render_template('de_ether.html')

@app.route('/monero/' , methods=["POST" , "GET"])
def monero():

    return render_template('de_monero.html')

@app.route('/usdt/' , methods=["POST" , "GET"])
def usdt():
...
def btc():
    ...
    return render_template('de_btc.html')


@app.route('/copy_address/' , methods=["POST" , "GET"])
def copy_address():
...
@app.route('/confirm_pdf/' , methods = ['POST','GET'])
def confirm_pdf():
    pdf = FPDF()
    pdf.add_page()
...
@app.route('/complete_regist' , methods = ['POST' , 'GET'])
def complete_regist():
...
if __name__ == "__main__":
    app.secret_key = "Fucddggdgdfdgdrer5677u"
